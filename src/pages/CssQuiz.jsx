...
const CssQuiz = () => {
  const cards = useQuizCards("css");
  const [cardIdx, setCardIdx] = useState(0);
  const questions = cards[cardIdx] || [];
  const { answers, submitAnswer, score } = useQuiz("css", questions);
  const { resetQuiz } = useContext(QuizContext);
  const [showResults, setShowResults] = useState(false);

  return (
    <div className="min-h-screen p-8 flex items-center justify-center bg-gradient-to-br from-blue-100 via-white to-cyan-100">
      <div className="w-full max-w-xl bg-white/80 rounded shadow-lg p-8">
        <h1 className="text-2xl font-bold mb-4">CSS Quiz (Deutsch)</h1>
        <div className="flex gap-4 mb-6">
          <button
            className="px-4 py-2 bg-blue-500 hover:bg-blue-600 text-white rounded transition"
            onClick={() => resetQuiz("css")}
          >
            Neu starten
          </button>
          <button
