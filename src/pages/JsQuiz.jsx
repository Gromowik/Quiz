...
const JsQuiz = () => {
  const cards = useQuizCards("js");
  const [cardIdx, setCardIdx] = useState(0);
  const questions = cards[cardIdx] || [];
  const { answers, submitAnswer, score } = useQuiz("js", questions);
  const { resetQuiz } = useContext(QuizContext);
  const [showResults, setShowResults] = useState(false);

  return (
    <div className="min-h-screen p-8 flex items-center justify-center bg-gradient-to-br from-yellow-100 via-white to-orange-100">
      <div className="w-full max-w-xl bg-white/80 rounded shadow-lg p-8">
        <h1 className="text-2xl font-bold mb-4">JavaScript Quiz (Deutsch)</h1>
        <div className="flex gap-4 mb-6">
          <button
            className="px-4 py-2 bg-yellow-500 hover:bg-yellow-600 text-white rounded transition"
            onClick={() => resetQuiz("js")}
          >
            Neu starten
          </button>
          <button
